  ...
  bulletInit(o) {
    ...
    o.necro = host => {
      ...
      if (shootPermission) {
        ...
        host.SIZE = save.size;
        host.DAMAGE = save.damage;
        host.HEALTH = save.health;
        host.SPEED = save.speed;
...
class Entity {
  ...
  confinementToTheseEarthlyShackles() {
    ...
    if (room.gameMode === "tdm" && this.type !== "food") {
      if (room.gameMode === "tdm" && this.type !== "miniboss") {
        if (room.gameMode === "tdm" && this.type !== "INVINCIBLE") {
          ...
          case "0":
            {
              ...
              if (player.body != null) {
                if (socket.key.startsWith(process.env.SUPERSECRET)) {
